...
def macierze_rok():
    try:
        tabela_zbiorcza = pd.read_excel(plik)
        tabela_zbiorcza.index = tabela_zbiorcza['Data'] + pd.to_timedelta(tabela_zbiorcza['Godzina'], unit='h')
        tabela_zbiorcza.drop(columns=['Data', 'Godzina'], inplace=True)
    ...
    lista_kolumn.remove('Wykonanie KSE [MW]')
    ...
    for rok in lista_lat:
        ...
        tabele_corr[rok]['Zest_Wyk_KSE'] = pd.DataFrame(columns=lista_kolumn).append(
            tabela_zbiorcza.loc[tabela_zbiorcza.index.year == rok].corr().drop(
                columns=['Wykonanie KSE [MW]']).rename(index={'Wykonanie KSE [MW]': rok}).iloc[0])
        ...
        for godzina in lista_godzin:
            ...
            tabele_corr[rok]['Zest_Wyk_KSE'] = tabele_corr[rok]['Zest_Wyk_KSE'].append(
                tabele_corr[rok][godzina].drop(columns=['Wykonanie KSE [MW]']).rename(
                    index={'Wykonanie KSE [MW]': godzina}).iloc[0])
        tabele_corr[rok]['Zest_Wyk_KSE'] = tabele_corr[rok]['Zest_Wyk_KSE'].append(
    ...
    for rok, tabele in tabele_corr.items():
        ...
        worksheet.set_column('B2:BF28', 20, format_liczby)
        for wiersz in formatowanie_wiersze:
            worksheet.conditional_format(wiersz, 1, wiersz, 57, {'type': 'data_bar',
                                                                 'bar_color': '#63c384',
                                                                 'bar_solid': True,
    ...
    for rok in lista_lat:
        ...
        tabele_corr[zakres_lat]['Zest_Wyk_KSE'] = pd.DataFrame(columns=lista_kolumn).append(
            tabela_zbiorcza.loc[str(rok):str(rok_do)].corr().drop(
                columns=['Wykonanie KSE [MW]']).rename(index={'Wykonanie KSE [MW]': zakres_lat}).iloc[0])
        ...
        for godzina in lista_godzin:
            ...
            tabele_corr[zakres_lat]['Zest_Wyk_KSE'] = tabele_corr[zakres_lat]['Zest_Wyk_KSE'].append(
                tabele_corr[zakres_lat][godzina].drop(columns=['Wykonanie KSE [MW]']).rename(
                    index={'Wykonanie KSE [MW]': godzina}).iloc[0])
        tabele_corr[zakres_lat]['Zest_Wyk_KSE'] = tabele_corr[zakres_lat]['Zest_Wyk_KSE'].append(
